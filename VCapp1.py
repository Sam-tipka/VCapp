...
try:
    ...
    os.environ["ANTHROPIC_API_KEY"] = "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"
...
Focus on real, verifiable deals with actual amounts. Include the most relevant comparables."""
        ...
        with st.spinner("Scanning deal databases…"):
            try:
                raw = stream_claude(prompt, system, max_tokens=1500, use_search=True, model="claude-sonnet-4-6")
                raw = raw.strip()
                if raw.startswith("```"):
                    raw = raw.split("```")[1]
                    if raw.startswith("json"): raw = raw[4:]
